...
def create_dset(config):
    ...
    if 'eeg' in config.dset:
        if config.dset == "toy_eeg":
            ...
            data_path = "./data/Generated EEG/eeg_generated_9chs_sub_0.pkl"
...
def main(args):
    # add
    args.run_name, _, _ = read_config_file()
    args.run_id = ''.join([str(random.randint(0,9)) for _ in range(9)])
    write_run_id(args.run_id)
    ...
    if args.wandb:
        ...
        STF_LOG_DIR=f"./plots_checkpoints_logs/{args.run_name}/wandb_logs/{args.run_id}"  # mod
