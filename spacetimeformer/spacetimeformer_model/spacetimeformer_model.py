...
class Spacetimeformer_Forecaster(stf.Forecaster):
    def __init__(
        ...
        global_self_attn: str = "performer",  # mod performer
        local_self_attn: str = "performer",  # mod performer
        global_cross_attn: str = "performer",  # mod performer
        local_cross_attn: str = "performer",  # mod performer
        ...
        # add
        self.run_name, self.run_type, self.run_id = read_config_file()
    ...
    def step(self, batch: Tuple[torch.Tensor], train: bool):
        ...
        scaled_batch, unscaled_batch =  batch  # add

        # compute all loss values
        loss_dict = self.compute_loss(
            batch=scaled_batch,  # mod it was batch
            time_mask=time_mask,
        ...
        *_, y_t = scaled_batch  # mod it was batch
        ...
        stats["loss"] = (
            ...
            #+ self.class_loss_imp * loss_dict["class_loss"]  # mod commented
        ...
        #PRINT stats
        #print("\nclass_loss_imp: ", self.class_loss_imp)
        #print("recon_loss_imp: ", self.recon_loss_imp)
        #print("\nforecast_loss: ", stats["forecast_loss"])
    ...
    def compute_loss(self, batch, time_mask=None, forward_kwargs={}):
        ...
        forecast_loss, forecast_mask, channels_loss = self.forecasting_loss(  # mod
        ...
        if self.recon_loss_imp > 0:
            ...
            recon_loss, recon_mask, channels_loss = self.forecasting_loss(  # mod
        ...
        if self.run_type == 'test':
            folder = f"./plots_checkpoints_logs/{self.run_name}/plots/{self.run_id}/{self.run_type}"
            filename = '/channels_loss_val.pkl'
            if not os.path.exists(folder):
                os.makedirs(folder)
                #print("_____________ENTRATOOOOO__________0")
            if not os.path.exists(folder+filename):
                with open(folder+filename, 'wb') as file:
                    pickle.dump([], file)
                #print("_____________ENTRATOOOOO__________1")
            #print("_____________ENTRATOOOOO__________2")
            with open(folder+filename, 'rb') as file:
                channels_loss_list = pickle.load(file)
                channels_loss_list.append(channels_loss)
            with open(folder+filename, 'wb') as file:
                pickle.dump(channels_loss_list, file)
            #print("_____________ENTRATOOOOO__________3:", channels_loss_list)
        ...
        if self.embed_method == "spatio-temporal" and self.class_loss_imp > 0:
            ...
            class_loss, acc = 0.0, -1.0  # mod commented
            #class_loss, acc = -1.0, -1.0  # mod
        ...
        return {
            ...
            "channels_loss": channels_loss  # add
        }
    ...
    def forward_model_pass(self, x_c, y_c, x_t, y_t, output_attn=False):
        ...
        '''
        run_type = ''
        if self.run_name[0:5] == 'train':
            run_type = 'train'
        elif self.run_name[0:4] == 'test':
            run_type = 'test'
        else:
            raise Exception("MYERROR: Unknown run type")
        '''
        ...
        with torch.no_grad():
            ...
            if self.run_type == 'train':
                ...
                random_number = random.randint(0, 3000)  # 0 8
            if random_number == 3001:
                print("PLOTTING...")
                y_c = y_c.to(torch.device("cpu"))
                #print(batch_x_cpu[0,0])
                #print(batch_x_cpu[0,0]==[ 0.2355,  0.4818,  0.1407,  0.0654,  0.1606])
                y_t = y_t.to(torch.device("cpu"))
                #output_cpu = output.mean.to(torch.device("cpu"))
                print(f"type(forecast_output) ", type(forecast_output))
                #forecast_output = (forecast_output.sample()).to(torch.device("cpu"))  # mod
                forecast_output = forecast_output.to(torch.device("cpu"))  # mod
                
                # Plotting a prediction
                now = time.time()
                if not os.path.exists(f"{folder}/{now}"):
                    os.makedirs(f"{folder}/{now}")
                CHANNELS = int(cf.read('num_channels'))
                for ch in range(CHANNELS):
                    plt.plot([*y_c[0,:,ch], *y_t[0,:,ch]])
                    plt.plot([*y_c[0,:,ch], *forecast_output[0,:,ch]])
                    plt.savefig(f"{folder}/{now}/context_and_predictions_ch_{ch}.png")
                    plt.clf()

                enc_y = enc_y.to(torch.device("cuda:0"))  # mod
                dec_y = dec_y.to(torch.device("cuda:0"))  # mod
                forecast_output = forecast_output.to(torch.device("cuda:0"))  # mod
