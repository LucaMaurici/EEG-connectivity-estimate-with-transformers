...
class Spacetimeformer_Forecaster(stf.Forecaster):
    ...
    def forward_model_pass(self, x_c, y_c, x_t, y_t, output_attn=False):
        ...
        #mod
        folder = "pred_during_training_8_test"
        if not os.path.exists(f"./plots/{folder}"):
            os.mkdir(f"./plots/{folder}")
        with torch.no_grad():
            if random.randint(0, 5475) == 0:
                ...
                now = time.time()
                if not os.path.exists(f"./plots/{folder}/{now}"):
                    os.mkdir(f"./plots/{folder}/{now}")
                CHANNELS = 5
                for ch in range(CHANNELS):
                    plt.plot([*y_c[0,:,ch], *y_t[0,:,ch]])
                    plt.plot([*y_c[0,:,ch], *forecast_output[0,:,ch]])
                    plt.savefig(f"./plots/{folder}/{now}/context_and_predictions_ch_{ch}.png")
                    plt.clf()
