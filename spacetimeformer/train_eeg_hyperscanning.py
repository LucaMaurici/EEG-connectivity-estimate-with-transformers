...
def create_callbacks(config):
    saving = pl.callbacks.ModelCheckpoint(
        ...
        mode="min",
        filename=f"" + "{epoch:02d}-{val/norm_mse:.2f}", #mod val/mse
        save_top_k=1,
    )
    callbacks = [saving]
    ...
    for i, subject in enumerate(subjects):
        for condition in conditions:
            ...
            for channel in range(CHANNEL_START, CHANNEL_END+1):
                ...
                write_run_id(f"ch{channel}_sub_{cf.read('subject_id')}_{condition}")
                ...
                ########### ATTENTION ###########
                '''
                print("File list: ", get_list_of_files(f'./plots_checkpoints_logs/{args.run_name}/checkpoints/{args.run_id}/'))
                list_of_files = get_list_of_files(f'./plots_checkpoints_logs/{args.run_name}/checkpoints/{args.run_id}/')
                index = index_of_not_substring(list_of_files, 'desktop.ini')
                print(index)
                checkpoint_path = get_list_of_files(f'./plots_checkpoints_logs/{args.run_name}/checkpoints/{args.run_id}/')[index]
