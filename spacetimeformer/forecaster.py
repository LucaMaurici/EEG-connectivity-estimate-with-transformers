...
class Forecaster(pl.LightningModule, ABC):
    ...
    def zero_one_normalization(self, data):
        return torch.div(torch.sub(data, torch.min(data)), torch.sub(torch.max(data), torch.min(data)))
    ...
    def loss_fn(
        ...
        if self.loss == "mse":
            ...
            #channels_loss = (mask * (true - preds)).square() / max(mask.sum(), 1)  # add
        elif self.loss == "mae":
        ...
        #preds = self._inv_scaler(array=preds, scaler=None)  # add
        #true = self._inv_scaler(array=true, scaler=None)  # add
        #preds = self.zero_one_normalization(preds)
        #true = self.zero_one_normalization(true)

        # add
        num = 2.0 * abs(preds - true)
        den = abs(preds.detach()) + abs(true) + 1e-5
        channels_loss = [100.0 * (num / den).squeeze()]
        channels_loss.append(torch.abs(true - preds).squeeze())
        channels_loss.append((true - preds).square().squeeze())
        channels_loss = torch.stack(channels_loss)

        return loss, channels_loss  # mod
    ...
    def forecasting_loss(
        ...
        forecasting_loss, channels_loss = self.loss_fn(y_t, outputs, full_mask)  # mod
        return forecasting_loss, full_mask, channels_loss  # mod
    ...
    def compute_loss(
        ...
        loss, mask, channels_loss = self.forecasting_loss(
            outputs=outputs, y_t=y_t, time_mask=time_mask
        )
        return loss, outputs, mask, channels_loss
    ...
    def predict(
        ...
        y_c = y_c.to(self.device).float()  # mod
        x_t = x_t.to(self.device).float()
        # move y_c to cpu if it isn't already there, scale, and then move back to the model device
        #y_c = torch.from_numpy(self._scaler(y_c.cpu().numpy())).to(self.device).float()  # mod commented
    ...
    def _compute_stats(self, pred: torch.Tensor, true: torch.Tensor):
        ...
        scaled_pred = pred  # mod
        scaled_true = true  # mod
        stats = {
    ...
    def step(self, batch: Tuple[torch.Tensor], train: bool = False):
        ...
        loss, output, mask, channels_loss = self.compute_loss(
            batch=batch,
            time_mask=time_mask,
