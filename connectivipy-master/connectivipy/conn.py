...
class GCI(Connect):
    ...
    def calculate(self, data, gcimethod='yw', gciorder=None):
        ...
        print(f"\n----vrfull----\n{vrfull}")
        #variances = self.evaluate(arfull, data)
        gcval = np.zeros((k, k))
        for i in range(k):
            arix = [j for j in range(k) if i != j]
            ar_i, vr_i = Mvar().fit(data[arix, :], gciorder, gcimethod)
            for e, c in enumerate(arix):
                gcval[c, i] = np.log(vrfull[i, i]/vr_i[e, e])  # originale
                #print(f"\n----vr_i----\n{vr_i}")
                #gcval[c, i] = np.log(vr_i[e, e]/vrfull[i, i]) # modificato
        print(f"\n----gcval----\n{gcval}")
        return np.tile(gcval, (2, 1, 1))

    def evaluate_multitrial(self, ar, data):
        ar = ar.squeeze()
        #ar.shape: order channel channel
        print(f"ar.shape: {ar.shape}")
        #data.shape: channels samples trials
        data = np.transpose(data, (0,2,1))
        #data.shape: channels trials samples
        channels_losses = []
        for ch in range(data.shape[0]):
            channel_losses = []
            for trial in range(data.shape[1]):
                for sample_start in range(data.shape[2] - ar.shape[0]):  # samples - order
                    
                    data_window = data[ch, trial, sample_start:(sample_start+ar.shape[0])]
                    data_window_next = data[ch, trial, sample_start+ar.shape[0]]
                    #print(f"\n----data_window----\n{data_window}")
                    #print(f"\n----data_window.shape----\n{data_window.shape}")
                    pred = []
                    for ch2 in range(data.shape[0]):
                        pred.append(np.dot(ar[:,ch,ch2], np.flip(data[ch2, trial, sample_start:(sample_start+ar.shape[0])])))
                    #pred = np.dot(np.transpose(ar[:,:,ch]), np.flip(data_window))  # np.dot(channels samples, samples)
                    #print(f"\n----pred.shape----\n{pred.shape}")
                    pred = np.sum(pred)
                    #print(f"\n----pred----\n{pred}")
                    #print(f"\n----pred.shape----\n{pred.shape}")
                    #channel_losses.append((pred-data_window_next)**2)
                    channel_losses.append(data_window_next-pred)
                
            channels_losses.append(channel_losses)

            print(f"\n----channels_losses.shape----\n{np.shape(channels_losses)}")

        return np.var(channels_losses, axis=1)
        
        '''
        data = np.transpose(data, (2,1,0))
        #shape: trials samples channels  
        for trial in range(data.shape[0]):
            for sample_start in range(data.shape[1] - ar.shape[0]):
                print(ar.shape)
                #ar.shape: order channels channels
                data_window = data[trial, sample_start:sample_start+ar.shape[0], :]
                print(f"\n----data_window----\n{data_window}")
                print(f"\n----data_window.shape----\n{data_window.shape}")
                pred = np.dot(np.transpose(ar, [1,0,2]), data_window)
                print(f"\n----pred----\n{pred}")
                print(f"\n----pred.shape----\n{pred.shape}")
                break
            break
    ...
    def calculate_multitrial(self, data, gcimethod='yw', gciorder=None):
        """
        Granger Causality Index calculation from MVAR model.
        Args:
          *data* : numpy.array
        ...
        arfull, vrfull = Mvar().fit(data, gciorder, gcimethod)
        print(f"\n----arfull----\n{arfull.shape}")
        vrfull = np.absolute(vrfull)
        #print(f"\n----vrfull----\n{vrfull}")
        variances_full = self.evaluate_multitrial(arfull, data)
        print("variances_full: ", variances_full)
        gcval_lib = np.zeros((k, k))
        gcval = np.zeros((k, k))
        for i in range(k):
            arix = [j for j in range(k) if i != j]
            ar_i, vr_i = Mvar().fit(data[arix, :, :], gciorder, gcimethod)
            variances_i = self.evaluate_multitrial(ar_i, data[arix, :, :])
            vr_i = np.absolute(vr_i)
            for e, c in enumerate(arix):
                #gcval_lib[c, i] = np.log(vrfull[i, i]/vr_i[e, e])  # originale
                #print(f"\n----vr_i----\n{vr_i}")
                gcval_lib[c, i] = np.log(vr_i[e, e]/vrfull[c, c]) # modificato
                gcval[c, i] = np.log(variances_i[e]/variances_full[c]) # modificato con variances
        print(f"\n----gcval----\n{gcval}")
        return gcval, gcval_lib, np.sum(np.array(arfull), axis=0)
        #print(f"\n----arfull_sum----\n{type(np.sum(arfull, axis=0))}")
        #return np.sum(np.array(arfull), axis=0)
